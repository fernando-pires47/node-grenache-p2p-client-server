...
const lockKey = 'sell-transaction';
...
function handle(payload) {
  try{
    ...
    if(payload.type != 'sell') return replyUtils.error(new Error('invalid type'));
    ...
    // cannot exist another sell transaction for this e2e
    const alreadyExistSellTranByType = transactionsByIdE2e.find(p => p.type == 'sell');
    ...
    // if the buy tran has another ref, it's wrong 
    ...
    return replyUtils.success('Created sell transaction successfully');
  }finally {
